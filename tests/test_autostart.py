...
def test_the_bookmark_watcher_switches_on_when_a_profile_exists(psok_home, monkeypatch, tmp_path):
...
def test_no_profile_means_no_watcher(psok_home, monkeypatch, tmp_path):
...
def test_a_disabled_watcher_survives_a_reboot(psok_home, monkeypatch, tmp_path):
...
def test_the_x_reader_needs_both_the_binary_and_the_cookies(psok_home, monkeypatch):
...
def test_a_denied_reader_survives_a_reboot(psok_home, monkeypatch):
    """`psok social deny x` is a refusal, not merely an empty allowlist.
    ...
    set_secret("psok/x_auth_token", "a-cookie")
    set_secret("psok/x_ct0", "another-cookie")
...
def test_auto_setup_is_idempotent(psok_home, monkeypatch, tmp_path):
