...
def test_policy_defaults_written_on_first_load(amethyst_home):
    policy = SandboxPolicy.load()
    assert policy.enabled
    assert any(".ssh" in p for p in policy.denied_read_paths)
    assert (amethyst_home / "config" / "sandbox.yaml").exists()


def test_direct_mode_is_never_wrapped(amethyst_home):
    argv, backend = wrap_command("echo x", SandboxPolicy(enabled=False), "/tmp")
    assert backend is None and argv[0] == "/bin/bash"


@requires_sandbox
def test_wrapped_command_uses_the_platform_backend(amethyst_home):
...
@requires_sandbox
async def test_sandbox_masks_denied_credential_paths(db, amethyst_home, tmp_path):
    """The containment claim itself: a denied path must not be readable."""
    fake_home = tmp_path / "home"
    (fake_home / ".ssh").mkdir(parents=True)
    (fake_home / ".ssh" / "id_rsa").write_text("PRIVATE KEY")

    policy_file = amethyst_home / "config" / "sandbox.yaml"
...
@requires_sandbox
async def test_sandbox_still_allows_ordinary_work(db, amethyst_home, tmp_path):
    ctx = ToolContext(workspace_root=str(tmp_path))
