...
to the machine PSOK runs on -- has a failure mode worth stating before anything
...
free D1 queue, sends the receipt, and keeps the token alive. PSOK collects from
...
npx wrangler d1 create psok-relay        # put the id in wrangler.jsonc
npm run schema
...
That is the real prize. Meta talks to the Worker; PSOK talks *out* to the Worker.
No inbound port, no tunnel, no NAT hole, no exact-path bypass rule to get wrong.
Keep `psok serve` on `127.0.0.1` and there is nothing to publish.
...
this. **PSOK has no login and is not meant to have one** (ADR-0001), so the
...
Point it at the one `psok serve` process, never the Vite dev server. Then an
Access application covering `psok.example.com`, policy *Allow* -> emails -> your
address.
...
Keep `psok serve` bound to `127.0.0.1`. `psok serve --host 0.0.0.0` prints a
warning saying exactly this, and `psok doctor` reports which endpoints are on and
...
```
URL     https://psok-relay.<you>.workers.dev/share
Method  POST
...
the three loopback OAuth callbacks (PSOK's `:33418`, workspace-mcp's `:8765`,
...
`PSOK_OAUTH_CALLBACK_BIND=0.0.0.0` so PSOK's own sign-in survives. Google
...
**Two volumes, because connector state lives outside `PSOK_HOME`:**

- `./data/psok:/data/psok` — the database, the library and its media, config,
  logs, skills, `secrets.json`. A bind mount: it is yours to browse, back up
  and grep.
- `psok-connector-home:/home/psok` — token caches written by the connector
...
| shell sandbox | `bwrap` may fail on hardened hosts (Ubuntu 24.04 userns restriction) → commands run direct, the container is the boundary, `psok doctor` says so | `backend/security/sandbox.py` |
| bookmark watcher | finds no browser profile; the loop no-ops | `backend/browser/places.py` |
| office conversion | named error, no `soffice`; add it with `docker compose exec psok apt-get install -y libreoffice-writer` | `backend/tools/builtin/convert.py` |
...
The release gate: `docker run --rm --entrypoint sh psok:local -c 'ls -A
/data/psok /home/psok'` must print nothing — those directories are created
