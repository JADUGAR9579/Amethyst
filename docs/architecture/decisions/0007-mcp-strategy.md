...
LibreChat's MCP layer is production-hardened for a multi-tenant deployment: four transports, four trust tiers, OAuth on-behalf-of token exchange, per-user connection pools, and cluster-coordinated boot-time discovery. AMETHYST is single-user and single-process. See [mcp.md](../mcp.md).
...
- **Adopt LibreChat's MCP layer as-is.** Rejected: the excluded machinery exists to solve multi-tenancy problems AMETHYST does not have, and would be maintained indefinitely for no benefit.
...
The two-tier trust collapse removes LibreChat's schema-level restriction on user-submitted servers (no stdio, no variable interpolation). Because there is no separate untrusted submitter in AMETHYST, this restriction protects nothing; the underlying fact it encodes — stdio grants arbitrary command execution — is instead handled by [security.md](../security.md)'s first-call-to-new-server confirmation requirement.

## Consequences

MCP support without a distributed-systems layer. Adding an MCP server is a YAML edit. If a server needs OAuth, AMETHYST supports single-user authorization-code login into the same keychain-backed credential store integrations use, not the on-behalf-of exchange.
