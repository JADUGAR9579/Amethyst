...
These four words are used loosely across the ecosystem AMETHYST borrows from. This document fixes their meaning inside AMETHYST. Getting these boundaries wrong is how systems end up with three overlapping plugin mechanisms, so the definitions here are deliberately narrow and the rule for choosing between them is deliberately mechanical.

## Tool

**A tool is a single atomic capability with a fixed JSON-Schema contract, executed in-process by AMETHYST's own code.**
...
A skill lives at `~/.amethyst/skills/<name>/` and contains a required `SKILL.md` with YAML frontmatter, plus optional `scripts/`, `references/`, and `assets/` directories.
...
**An MCP tool is a tool whose implementation lives in an external process AMETHYST does not own, reached over the Model Context Protocol.**
...
|---|---|---|---|---|---|
| **Tool** | Yes, directly | AMETHYST, in-process | No | No | Writing code + registering |
...
| **Agent** | No — it *is* the caller | AMETHYST | No | Conversation | (v1: one, fixed) |
...
**1. Can AMETHYST's own backend do this directly against a local resource — filesystem, shell, local database, scheduling?**
...
→ **Configure an MCP server.** This is how AMETHYST reaches anything outside the local machine.
