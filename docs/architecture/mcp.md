...
LibreChat's MCP layer is production-hardened well beyond spec compliance, but a large fraction of its complexity exists specifically because it serves many users who are not the operator: per-user connection pools, on-behalf-of token exchange, cluster leader election, admin-versus-user trust tiers. AMETHYST has one user who is also the operator of every MCP server it configures. This document keeps the parts of LibreChat's design that hold value regardless of tenancy, and drops the parts that only make sense at LibreChat's scale.

## What AMETHYST keeps

**Three transports: stdio, SSE, streamable-HTTP.** LibreChat supports four, adding WebSocket for breadth. No commonly used MCP server currently requires WebSocket specifically; AMETHYST adds it if and when a real server does, rather than carrying an unused transport implementation and its test surface from day one.

**SSRF protection on URL-based transports.** Private-IP and CIDR-range blocking, applied at transport construction so a malicious or misconfigured server URL never establishes a connection. This matters precisely because AMETHYST is trusted-by-default (see below) — the guardrail here is not "is this server trustworthy" but "does this URL actually point somewhere the user intended," which protects against a bad URL reaching internal network services regardless of trust level.

**Composite tool-key namespacing.** Every discovered tool is registered as `{tool_name}__mcp__{server_name}`, so two servers offering same-named tools never collide in AMETHYST's one flat tool namespace. This is a one-line policy decision with a real payoff once more than one server is configured.
...
**In AMETHYST those are the same person.** There is no untrusted submitter to defend against, so AMETHYST collapses to two categories, both full trust:
...
- **`bundled`** — first-party-recommended servers AMETHYST ships templates for (for instance a browser-automation server), offered during onboarding rather than silently auto-installed.

There is no third, restricted tier, because restricting configuration the user wrote for themselves protects against nothing. What the two-tier collapse does **not** remove is the underlying fact LibreChat's restricted schema encodes correctly: **stdio grants arbitrary command execution.** That fact is documented in AMETHYST's threat model and enforced through the [security](security.md) layer's first-call-to-new-server confirmation, rather than through a config-schema restriction that would have no one to restrict.

## What AMETHYST drops entirely

- **On-behalf-of token exchange.** Exists to convert one user's identity into a downstream-scoped token for a different user's request in a shared deployment. AMETHYST's every credential belongs to the same person; there is no downstream identity to scope to.
...
If a configured MCP server itself requires OAuth, AMETHYST supports plain single-user OAuth2 authorization-code login, once per server, with the resulting token stored in the same keychain-backed credential store Integrations use — not the on-behalf-of machinery.

## Configuration

```yaml
# ~/.amethyst/config/mcp.yaml
mcpServers:
  chrome-devtools:
    transport: stdio
    command: npx
...
4. A call to an MCP tool goes through the identical dispatch path as any other tool call: risk-floor lookup (MCP tools default to at least medium risk, since AMETHYST cannot inspect what an external server actually does), first-call-to-server confirmation if this server has not been used before, execution over the transport with the connection's timeout, result normalization, and audit logging.
...
This restates the relevant branch of the decision rule in [components.md](components.md): reach for MCP when a good server already exists for an on-demand capability that AMETHYST does not need to persist data from locally — browser automation, an ad-hoc web API, a niche one-off service. Build a builtin tool or an Integration instead when the data needs to live in AMETHYST's own database to be cross-referenced with tasks, calendar, or memory, or when AMETHYST wants to own the credential lifecycle directly rather than delegate it to a server's own auth handling.
